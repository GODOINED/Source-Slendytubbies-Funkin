...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
			...
			case 'stage':
				{
						defaultCamZoom = 1;
						curStage = 'stage';
						var bg:FlxSprite = new FlxSprite(-300, -400).loadGraphic(Paths.image('stageback'));
						...
						var stageCurtains:FlxSprite = new FlxSprite(-420, -20).loadGraphic(Paths.image('stagecurtains'));
						stageCurtains.setGraphicSize(Std.int(stageCurtains.width * 0.78));
				...
				case 'Outskirts':
					{
							defaultCamZoom = 0.85;
							curStage = 'Outskirts';
							var bg:FlxSprite = new FlxSprite(-600, -150).loadGraphic(Paths.image('LaaLaa/stageback'));
							bg.setGraphicSize(Std.int(bg.width - 1.3));
							bg.antialiasing = true;
							...
							var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('LaaLaa/stagefront'));
							stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
							stageFront.updateHitbox();
							stageFront.antialiasing = true;
							stageFront.scrollFactor.set(0.9, 0.9);
							...
							var stageCurtains:FlxSprite = new FlxSprite(-500, -300).loadGraphic(Paths.image('LaaLaa/stagecurtains'));
		...
		if (!PlayStateChangeables.Optimize)
		{
			...
			if (curStage == 'Outskirts')
				add(dad);
				add(boyfriend);

            if (curStage == 'stage')
            {
                var dark:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('vignettatura')); //DARK Don't Delete
				var dark2:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('vignettatura2')); //DARK Don't Delete
				var noise:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('noise')); //Noise Don't Delete
				var fog:FlxSprite = new FlxSprite(0, 307);
                dark.antialiasing = true;
				dark2.antialiasing = true;
				noise.antialiasing = true;
                dark.scrollFactor.set(0, 0);
				dark2.scrollFactor.set(0, 0);
				noise.scrollFactor.set(0, 0);
				noise.frames = Paths.getSparrowAtlas("noise");
				noise.animation.addByPrefix('idle', "Processing", 24, true);
				...
				dark.alpha = 0.8;
				dark2.alpha = 0.95;
				fog.frames = Paths.getSparrowAtlas('fog');
				fog.animation.addByPrefix('idle', "Particle", 24, true);
				fog.animation.play('idle');
                fog.antialiasing = true;
                fog.scrollFactor.set(0, 0.26);
                fog.updateHitbox();
                fog.alpha = 0.2;
				add(dark);
				add(fog);
			...
			if (curStage == 'Outskirts')
				{
					var dark:FlxSprite = new FlxSprite(-123, -70).loadGraphic(Paths.image('vignettatura')); //DARK Don't Delete
					var dark2:FlxSprite = new FlxSprite(-123, -70).loadGraphic(Paths.image('vignettatura2')); //DARK Don't Delete
					var noise:FlxSprite = new FlxSprite(-123, -70).loadGraphic(Paths.image('noise')); //Noise Don't Delete
					var fog:FlxSprite = new FlxSprite(-123, 267); //FOG Don't Delete
					dark.antialiasing = true;
					dark2.antialiasing = true;
					noise.antialiasing = true;
					noise.scrollFactor.set(0, 0);
					...
					dark.scrollFactor.set(0, 0);
					dark2.scrollFactor.set(0, 0);
					dark.setGraphicSize(Std.int(dark.width * 1.2));
					dark2.setGraphicSize(Std.int(dark2.width * 1.2));
					noise.setGraphicSize(Std.int(noise.width * 1.2));
					dark.updateHitbox();
					dark2.updateHitbox();
					noise.updateHitbox();
					noise.alpha = 0.25;
					...
					fog.antialiasing = true;
					fog.scrollFactor.set(0, 0.26);
					fog.setGraphicSize(Std.int(fog.width * 1.2));
					fog.updateHitbox();
					fog.alpha = 0.1;
					add(dark);
					add(fog);
		...
		redscreenshit = new FlxSprite().loadGraphic(Paths.image('reddead'));
	...
	override public function update(elapsed:Float)
	{
		...
		perfectMode = false;
		#end
		if (curSong == 'Shock' && storyDifficulty >= 2) //totally not copying tabis code
        {
            redscreenshit.alpha = 1 - (health / 3);
        }
        else if (curSong == 'Shock') //but actually credits to Gweb
        {
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						switch (Math.abs(daNote.noteData)) //NoteDamage
						{
							case 2:
	...
	private function popUpScore(daNote:Note):Void
		{
			...
			function glitch() {
				var babyArrow:FlxSprite = new FlxSprite(0, strumLine.y);
				var numScore:FlxSprite = new FlxSprite(0, strumLine.y);
				var combospr:FlxSprite = new FlxSprite(0, strumLine.y);
                var endme:String = '';
                var screamnumber = FlxG.random.int(1,4);
                switch (screamnumber)
                {
                    case 1:
                        endme = 'Tinky';
                ...
                var glitch:FlxSprite = new FlxSprite(0, 0);
                var randomimage:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image("Screen/" + endme));
				randomimage.frames = Paths.getSparrowAtlas("Screen/" + endme);
				randomimage.animation.addByPrefix('idle', "Jump", 24, true);
                randomimage.updateHitbox();
                randomimage.scrollFactor.set(0, 0);
                glitch.frames = Paths.getSparrowAtlas('Screen/Glith');
                glitch.animation.addByPrefix('idle', 'Glith', 24, true);
                glitch.updateHitbox();
                glitch.antialiasing = false;
                glitch.scrollFactor.set(0, 0);
                glitch.active = true;
				healthBarBG.visible = false;
				kadeEngineWatermark.visible = false;
				healthBar.visible = false;
                ...
                FlxG.sound.play(Paths.sound('Glith'));
                glitch.animation.play('idle');
				randomimage.animation.play('idle');
				randomimage.alpha = 1;
                glitch.alpha = 0.1;
                new FlxTimer().start(0.3, function(tmr:FlxTimer){
					remove(glitch);
